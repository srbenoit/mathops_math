 ...
 */
public final class FloorOperator extends AbstractOperator {

    /**
     * Constructs a new {code FloorOperator}.
     */
    ...
    public final void execute(final OperatorStack stack) throws OperatorException {
        ...
        if (arg == null) {
            throw new OperatorException(EOperatorExceptionType.STACK_UNDERFLOW,
                    "[floor] operation executed when stack is empty.");
        }

        if (arg instanceof Double argDbl) {
            final double value = argDbl.doubleValue();
            final double floorValue = Math.floor(value);
            stack.pushDouble(floorValue);
     ...
     *
     * @return the representation ("floor")
     */
    @Override
